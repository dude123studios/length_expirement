...
def compute_and_render(example_idx: int):
    ...
    fig.add_trace(go.Scatter(
        x=list(range(len(model_answer_tokens))), y=answer_probs, mode="lines+markers",
        name="probs",
        hovertemplate="%{text}<br>y=%{y:.4f}",
    ...
    for i, tok in enumerate(model_answer_tokens_str):
        if normalize_token_for_match(tok) in SPECIAL_TOKENS:
            ...
            fig.add_annotation(
                x=i, y=max(answer_probs)*1.05,  # place above curve
                text=tok,
                showarrow=False,
