...
def parse_args():
    ...
    parser.add_argument('--temperatures', type=float, nargs='+', default=[0.0, 0.7, 1.5, 2.5],
...
def run_single_multi_temperature_experiment(model, tokenizer, trace_file, decision_token_ids, args, device):
    """Run multi-temperature experiment on a single trace file"""
    try:
        # Load trace
        ...
        for temp in args.temperatures:
            print(f"🌡️  Running temperature {temp}...")
            position, probs = generate_continuation(
                model, tokenizer, prefix_ids, temperature=temp, 
                max_new_tokens=args.max_new_tokens, decision_token_ids=decision_token_ids, 
                device=device, stop_on_high_likelihood=True
            )
            
            # Store results with temperature as key
            results[f'temp_{temp}_position'] = position
            results[f'temp_{temp}_probs'] = probs
...
def run_multi_temperature_experiment(args):
    ...
    if not results:
        ...
        results_file = os.path.join(args.output_dir, 'multi_temperature_results.json')
    ...
    temp_stats = {}
    for temp in args.temperatures:
        positions = [r[f'temp_{temp}_position'] for r in results]
        temp_stats[temp] = {
            'positions': positions,
    ...
    for temp in args.temperatures:
        ...
        print(f"   T={temp} range: {stats['min']} - {stats['max']}")
        print(f"   T={temp} hit max length: {sum(1 for p in stats['positions'] if p == args.max_new_tokens)}")
    
    print(f"\n📊 RESULTS:")
    for temp in args.temperatures:
        stats = temp_stats[temp]
        print(f"   T={temp}: {stats['mean']:.2f} ± {stats['std']:.2f}")
    ...
    # Create multi-temperature visualizations
    create_multi_temperature_plots(results, args.output_dir, temp_stats, args.temperatures)
    
    # Create CSV file with the data
    create_multi_temperature_csv(results, args.output_dir, args.temperatures)
...
def create_multi_temperature_csv(results, output_dir, temperatures):
    """Create CSV file with multi-temperature data"""
    print("📊 Creating multi-temperature results CSV file...")
    ...
    for i, result in enumerate(results):
        ...
        # Add columns for each temperature
        for temp in temperatures:
            row[f'temp_{temp}_length'] = result[f'temp_{temp}_position']
        
        csv_data.append(row)
    
    # Create DataFrame and save to CSV
    df = pd.DataFrame(csv_data)
    csv_file = os.path.join(output_dir, 'multi_temperature_results.csv')
    df.to_csv(csv_file, index=False)
    
    print(f"📊 Results CSV saved to {csv_file}")
    print(f"   Columns: example_id, file, start_position, temp_0.0_length, temp_0.7_length, temp_1.5_length, temp_2.5_length")
    print(f"   Rows: {len(csv_data)} examples")
    
    # Print summary statistics
    print(f"\n📈 CSV Summary:")
    for temp in temperatures:
        col_name = f'temp_{temp}_length'
        print(f"   T={temp} mean: {df[col_name].mean():.2f} ± {df[col_name].std():.2f}")
    
    return csv_file

def create_multi_temperature_plots(results, output_dir, temp_stats, temperatures):
    """Create multi-temperature visualization plots"""
    print("📊 Creating multi-temperature plots...")
    
    # Create a 2x2 subplot layout
    fig = make_subplots(
        rows=2, cols=2,
        subplot_titles=[
            'Length Distribution by Temperature',
            'Temperature Comparison (Box Plot)',
            'Length vs Temperature (Scatter)',
            'Statistical Summary'
        ],
        specs=[[{"secondary_y": False}, {"secondary_y": False}],
               [{"secondary_y": False}, {"secondary_y": False}]]
    ...
    # 1. Length Distribution by Temperature (Histogram)
    colors = ['blue', 'green', 'orange', 'red']
    for i, temp in enumerate(temperatures):
        positions = temp_stats[temp]['positions']
        fig.add_trace(
            go.Histogram(
                x=positions,
                name=f'T={temp}',
    ...
    # 2. Temperature Comparison (Box Plot)
    box_data = []
    for temp in temperatures:
        box_data.append(go.Box(
            y=temp_stats[temp]['positions'],
            name=f'T={temp}',
            marker_color=colors[temperatures.index(temp) % len(colors)]
        ))
    
    for box in box_data:
        fig.add_trace(box, row=1, col=2)
    
    # 3. Length vs Temperature (Scatter)
    for i, result in enumerate(results):
        x_vals = temperatures
        y_vals = [result[f'temp_{temp}_position'] for temp in temperatures]
        fig.add_trace(
            go.Scatter(
                x=x_vals,
                y=y_vals,
                mode='lines+markers',
                name=f'Example {i+1}' if i < 5 else None,  # Only show first 5 examples
                opacity=0.6,
                showlegend=i < 5
            ),
            row=2, col=1
    ...
    means = [temp_stats[temp]['mean'] for temp in temperatures]
    stds = [temp_stats[temp]['std'] for temp in temperatures]
    
    fig.add_trace(
        go.Bar(
            x=[f'T={temp}' for temp in temperatures],
            y=means,
    ...
    # Update layout
    fig.update_layout(
        title='Multi-Temperature Subthought Length Analysis',
        height=800,
        width=1200,
        showlegend=True
    )
    ...
    # Update axes labels
    fig.update_xaxes(title_text="Length (tokens)", row=1, col=1)
    fig.update_yaxes(title_text="Count", row=1, col=1)
    fig.update_yaxes(title_text="Length (tokens)", row=1, col=2)
    fig.update_xaxes(title_text="Temperature", row=2, col=1)
    fig.update_yaxes(title_text="Length (tokens)", row=2, col=1)
    fig.update_xaxes(title_text="Temperature", row=2, col=2)
    fig.update_yaxes(title_text="Mean Length (tokens)", row=2, col=2)
    
    # Save plot
    plot_file = os.path.join(output_dir, 'multi_temperature_analysis.html')
    fig.write_html(plot_file)
    print(f"📊 Multi-temperature analysis plot saved to {plot_file}")
    
    return plot_file

if __name__ == "__main__":
    args = parse_args()
    run_multi_temperature_experiment(args)
