#!/usr/bin/env python3
"""
MacBook Pro Optimized Subthought Length Temperature Experiment
...
def get_optimal_device():
    """Get the best available device for MacBook Pro."""
...
def generate_continuation_macbook(
    example_idx: int,
    ...
    """
    Generate continuation optimized for MacBook Pro.
    """
    ...
    # Ensure all tensors are on the correct device (MPS for MacBook Pro)
    ...
    # Check input length (conservative for MacBook Pro)
    if input_ids.size(1) > 2000:  # Conservative limit for MPS
        return max_new_tokens, ""
    
    # Generate - optimized for MacBook Pro
    do_sample = temperature > 0.0
    try:
        with torch.no_grad():
            # Use MPS-optimized generation parameters
            generation_kwargs = {
                "input_ids": input_ids,
...
def run_macbook_experiment(
    ...
    output_dir: str = "macbook_results"
) -> List[Dict]:
    """Run the MacBook Pro optimized experiment."""
    
    print(f"🍎 MacBook Pro Optimized Experiment")
    print(f"Setting up model: {model_name_or_path}")
    
    # Get optimal device for MacBook Pro
    ...
    # Load model with MacBook Pro optimizations
    print("Loading model with MacBook Pro optimizations...")
    model = AutoModelForCausalLM.from_pretrained(
        model_name_or_path, 
        device_map="auto", 
        torch_dtype=torch.float16,  # Use float16 for better MPS performance
        trust_remote_code=True,
        low_cpu_mem_usage=True,  # Optimize for MacBook Pro memory
    )
    model.eval()
    
    # Note: device_map="auto" already handles device placement
    print(f"Model loaded with device_map='auto' - device placement handled automatically")
    ...
    for text_example in tqdm(text_examples, desc="Processing examples"):
        try:
            ...
            for decision_pos in decision_positions[:2]:  # Limit to first 2 decision tokens
                continuation_index = decision_pos
                
                # Run greedy generation
                greedy_length, greedy_text = generate_continuation_macbook(
                    example_idx=example_idx,
                ...
                high_temp_length, high_temp_text = generate_continuation_macbook(
                    example_idx=example_idx,
                    text_examples=text_examples,
                    continuation_index=continuation_index,
                ...
                result = {
                    ...
                    "greedy_text": greedy_text[:100],  # Truncate for storage
                ...
                print(f"Example {example_idx}, Decision {decision_pos}: Greedy={greedy_length}, High-temp={high_temp_length}")
    ...
    results_file = os.path.join(output_dir, f"macbook_results_{benchmark_name}.json")
...
def analyze_macbook_results(results: List[Dict]) -> Dict:
    """Analyze results with MacBook Pro optimizations."""
    if not results:
        return {}
    ...
    stats_dict = {
        ...
        "device": results[0].get("device", "unknown") if results else "unknown"
    }
...
def main():
    parser = argparse.ArgumentParser(description="MacBook Pro Optimized Subthought Length Temperature Experiment")
    ...
    parser.add_argument('--output_dir', type=str, default="macbook_results", help="Output directory")
    
    args = parser.parse_args()
    
    # Run the experiment
    results = run_macbook_experiment(
    ...
    # Analyze results
    stats = analyze_macbook_results(results)
    
    print("\n🍎 MacBook Pro Analysis Results:")
    for key, value in stats.items():
        print(f"{key}: {value}")
    
    print("\nMacBook Pro optimized experiment completed successfully!")
