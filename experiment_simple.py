#!/usr/bin/env python3
"""
Simplified Subthought Length Temperature Experiment
Based on the working patterns from other experiments in the codebase.
"""
...
def generate_continuation(
    ...
    """
    Generate continuation from a specific index.
    Simplified version based on working examples.
    """
    ...
    # Check input length
    if input_ids.size(1) > 3000:  # Conservative limit
        return max_new_tokens, ""
    
    # Generate - using the exact pattern from working examples
...
def run_simple_experiment(
    ...
    output_dir: str = "simple_results"
) -> List[Dict]:
    """Run the simplified experiment."""
    
    print(f"Setting up model: {model_name_or_path}")
    
    # Setup - following exact pattern from working examples
    ...
    for text_example in tqdm(text_examples, desc="Processing examples"):
        try:
            ...
            for decision_pos in decision_positions[:2]:  # Limit to first 2 decision tokens
                ...
                greedy_length, greedy_text = generate_continuation(
                    example_idx=example_idx,
                ...
                high_temp_length, high_temp_text = generate_continuation(
                    example_idx=example_idx,
                    text_examples=text_examples,
                    continuation_index=continuation_index,
    ...
    results_file = os.path.join(output_dir, f"simple_results_{benchmark_name}.json")
...
def analyze_simple_results(results: List[Dict]) -> Dict:
...
def main():
    parser = argparse.ArgumentParser(description="Simple Subthought Length Temperature Experiment")
    ...
    parser.add_argument('--output_dir', type=str, default="simple_results", help="Output directory")
    ...
    # Analyze results
    stats = analyze_simple_results(results)
