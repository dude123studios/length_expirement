...
- PROPERLY starts from nonzero positions after decision tokens
- PROPERLY applies temperature without invalid flags
- PROPERLY detects decision tokens during generation
- PROPERLY handles model inputs and attention masks
"""
...
def generate_subthought_with_decision_detection(
    example_idx: int,
    ...
    """
    PROPERLY generate subthought starting AFTER a decision token, 
    with REAL-TIME decision token detection during generation.
    """
    ...
    # PROPERLY start AFTER the decision token (not at it)
    ...
    # PROPERLY generate with correct parameters - EXACT pattern from generate_activations.py
    try:
        with torch.no_grad():
            out = model.generate(
                input_ids=input_ids,
                ...
                top_p=0.9,  # Add top_p like in generate_activations.py
                do_sample=False if temperature == 0 else True,  # EXACT pattern from working code
                num_beams=1,
    ...
    # PROPERLY find first decision token in generated sequence
    special_token_ids = set(special_token_to_id.values())
    subthought_length = max_new_tokens  # Default if no decision token found
    
    # Check each generated token for decision tokens
    for i, token_id in enumerate(gen_ids.tolist()):
        if token_id in special_token_ids:
            subthought_length = i + 1  # +1 because we count the decision token
            print(f"🎯 FOUND decision token {token_id} at position {i}, stopping at length {subthought_length}")
            break
...
def run_final_experiment(
    ...
    print(f"🚀 FINAL Subthought Length Experiment")
    ...
    print(f"Special token mapping: {special_token_to_id}")
    ...
    print(f"Running FINAL experiment on {len(text_examples)} examples...")
    ...
    for text_example in tqdm(text_examples, desc="Processing examples"):
        try:
            ...
            print(f"\n📊 Example {example_idx}: Found decision tokens at positions {decision_positions}")
            ...
            for i, decision_pos in enumerate(decision_positions[:-1]):  # Exclude last decision token
                ...
                print(f"\n🎯 Processing example {example_idx}, starting AFTER decision position {decision_pos}")
                
                # Run greedy generation
                greedy_length, greedy_text = generate_subthought_with_decision_detection(
                    example_idx=example_idx,
                ...
                high_temp_length, high_temp_text = generate_subthought_with_decision_detection(
                    example_idx=example_idx,
                    text_examples=text_examples,
                    start_after_decision_pos=decision_pos,
    ...
    results_file = os.path.join(output_dir, f"final_results_{benchmark_name}.json")
...
def analyze_final_results(results: List[Dict]) -> Dict:
    """Analyze results."""
    if not results:
        return {}
...
def main():
    parser = argparse.ArgumentParser(description="FINAL Subthought Length Temperature Experiment")
    ...
    # Analyze results
    stats = analyze_final_results(results)
