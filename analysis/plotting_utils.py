...
def get_token_cosine_similarity_colored_html(
    ...
    for token, sim in zip(tokens, similarities):
        color = colors.to_hex(cmap(norm(sim)))  # RGBA -> #RRGGBB
        html += f"<span style='background-color:{color}; padding:2px; margin:1px; border-radius:2px;'>{token}</span> "
    return f"<div style='line-height:2.0; font-family:monospace'>{html}</div>"

def get_cosine_similarity_layer_by_layer(
    deltas: torch.Tensor,
    model_name: str = "Model 1",
):
    deltas = np.array(deltas)
    fig, ax = plt.subplots(figsize=(10, 4))
    im = ax.imshow(deltas.T, aspect='auto', cmap='RdYlGn', interpolation='nearest')
    ax.set_title(f"Activation Change per Token – {model_name}")
    ax.set_ylabel("Layer")
    ax.set_xlabel("Token Step")
    fig.colorbar(im, ax=ax, orientation='vertical', label="Cosine Similarity")
    plt.tight_layout()
